...
Deno.test("cache - reordering: selected entry should move to front without removing the original newest", () => {
...
Deno.test("cache - reordering: newest entry re-added should stay at front", () => {
  const cache = createYankCache(10)
...
Deno.test("cache - reordering: duplicate entries are allowed and added as new", () => {
  const cache = createYankCache(10)
...
Deno.test("cache - moveToFront: non-existent entry returns false", () => {
  const cache = createYankCache(10)
