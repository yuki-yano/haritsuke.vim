...
import type { SyncStatus, YankDatabase } from "../data/database.ts"
...
describe("createSyncManager", () => {
  it("no sync needed when status unchanged", async () => {
  ...
  it("syncs when timestamp changes", async () => {
  ...
  it("syncs when entry count changes", async () => {
    const initialStatus: SyncStatus = { lastTimestamp: 1000, entryCount: 5 }
  ...
  it("getLastStatus returns null initially", () => {
    const mockDb = createMockDatabase()
    const mockCache = createMockCache()
    const syncManager = createSyncManager(mockDb, mockCache, { maxEntries: 100 })
  ...
  it("getLastStatus returns status after updateStatus", () => {
  ...
  it("getLastStatus returns status after sync", async () => {
  ...
  it("logs sync operations with logger", async () => {
    const mockLogger = {
