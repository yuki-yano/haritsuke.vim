...
Deno.test("createHighlightManager - applies character mode highlight", async () => {
  const mockFns = {
    line: (expr: string) => {
      if (expr === "'[") return 10
      if (expr === "']") return 10
...
Deno.test("createHighlightManager - applies line mode highlight for single line", async () => {
  const mockFns = {
    line: (expr: string) => {
      if (expr === "'[") return 5
      if (expr === "']") return 5
...
Deno.test("createHighlightManager - applies line mode highlight for multiple lines", async () => {
  const mockFns = {
    line: (expr: string) => {
      if (expr === "'[") return 3
      if (expr === "']") return 5
...
Deno.test("createHighlightManager - applies block mode highlight", async () => {
  const mockFns = {
    line: (expr: string) => {
      if (expr === "'[") return 10
      if (expr === "']") return 15
...
Deno.test("createHighlightManager - clears previous highlight before applying new", async () => {
  const mockFns = {
    line: () => 1,
    col: () => 1,
    getregtype: () => "v",
...
Deno.test("createHighlightManager - clear removes active highlight", async () => {
  const mockFns = {
    line: () => 1,
    col: () => 1,
    getregtype: () => "v",
...
Deno.test("createHighlightManager - clear ignores error if match doesn't exist", async () => {
  const mockFns = {
    matchdelete: spy(() => {
...
Deno.test("createHighlightManager - isActive returns correct state", () => {
  const manager = createHighlightManager({})
...
Deno.test("createHighlightManager - handles errors gracefully", async () => {
  const mockLogger = {
